...
|---|---|---|
| `huelle.py` | Programm-Hülle (Spec Stufe 2, JB-Go 05.08.): eigenes Fenster (pywebview) lädt die Oberfläche vom lokalen Server; startet den Server selbst, falls er aus ist. Bettet das VLC-Video in ein WinForms-Panel ein (`/api/vlc` Befehl `fenster`: Anmeldung mit PID, vor jedem VLC-Start neu, Abmeldung beim Schließen; in derselben Anfrage pausiert der Server ein Video, das gerade in IHR Panel spielt — Musik läuft weiter, JB 24.09.) | Start über `SyncYouTube-Fenster.bat`; ruft `youtube_app.py` als Subprozess; js_api zeigt der Seite NUR Methoden (`video_rect`, `video_melden`) — öffentliche Daten ließen pywebview in das .NET-Geflecht laufen (Hänger 07.08./24.09.); Wächter `tests/test_huelle.py` |
